...
% An even more modified version of hyperbolic_2d.m file which calculates
% all Jnmp and their mean and plots their norms for psi and psi^2 .
clear all;
close all;
...
% Jmean
psi_h4 = psi_nm14 - (dt/2)*Jmean(chi, psi_nm1, dx);
psi_n4 = psi_nm14 - dt.*Jmean(chi, psi_h4, dx);

% Initialise our vectors to 0
psi_np11 = zeros(size(psi_n1));
psi_np12 = zeros(size(psi_n2));
psi_np13 = zeros(size(psi_n3));
psi_np14 = zeros(size(psi_n4));
t = zeros(1,N-1);
sumPsi1 = zeros(1,N-1);
sumPsi2 = zeros(1,N-1);
sumPsi3 = zeros(1,N-1);
...
sumSqPsi4 = zeros(1,N-1);

% We make our plots better looking
...
for n = 1:N - 1
    ...
    psi_np14(2:end-1,2:end-1) = psi_nm14(2:end-1,2:end-1)-2*dt*Jn4(2:end-1,2:end-1);
